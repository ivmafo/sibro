		...
		public String convertToString(double number){
			...
			for (int i = aux.size()-1; i >= 0; i--) {
				...
				if (Double.parseDouble(string)!=0){
					...
					String numConverted = ConvertNumberToString.convertNumberToLetter(string);
					if (string.endsWith("1") && MILLONES.get(contAux)!=null && string.length()==1)
						res = (numConverted.endsWith(" ") ? numConverted : numConverted + " ")+ MILLONES.get(contAux)[0] + " " + res;
					else if (MILLONES.get(contAux)!=null)
						res = (numConverted.endsWith(" ") ? numConverted : numConverted + " ") + MILLONES.get(contAux)[1] + " "+res;
					else
						res = (numConverted.endsWith(" ") ? numConverted : numConverted + " ") + res;
				}
				contAux++;
			}
			
			if (res.endsWith("LLONES ") || res.endsWith("LLON "))
				res += "DE PESOS M/CTE";
			else
				res += "PESOS M/CTE";
